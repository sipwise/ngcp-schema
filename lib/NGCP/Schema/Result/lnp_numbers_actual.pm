...
select ln.lnp_provider_id,ln.number,max(ln.id) as actual_ln_id from billing.lnp_numbers ln
    inner join
        (select lnn.lnp_provider_id,lnn.number, max(lnn.start) max_start
        ...
        group by lnn.lnp_provider_id,lnn.number
    ) lnn on ln.lnp_provider_id=lnn.lnp_provider_id and ln.number=lnn.number and (ln.start=lnn.max_start or (lnn.max_start is null and ln.start is null))
group by ln.lnp_provider_id, ln.number
");
